...
public interface IndexRepository extends JpaRepository<ModelIndex, Integer> {
    ...
    List<ModelIndex> findByPagesAndLemmas(@Param("lemmas") List<Lemma> lemmaListId,
                                          @Param("pages") List<Page> pageListId);
    List<ModelIndex> findByLemmaId (long lemmaId);
    List<ModelIndex> findByPageId (long pageId);
    ModelIndex findByLemmaIdAndPageId (long lemmaId, long pageId);
}
